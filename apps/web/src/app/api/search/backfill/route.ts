...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const records = posts.map((post: any) => ({
      ...
      authors: Array.isArray(post.authors)
        ? post.authors.map((a: any) =>
            String(typeof a === "object" ? a.name || "" : a).slice(0, 100)
          )
        : [],
    ...
    // Configure index settings in code per requirement 2b
    ...
    logger.info("Configured Algolia index settings (searchableAttributes, faceting, ranking)");
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully backfilled ${records.length} posts and configured index settings`,
